function [ data_out ] = RS_Giac_EEG_CreateElectrodes(data, channels_to_interpolate, layout_file_name, neighb_dist)
...
for ch_to_add = 1: length(channels_to_interpolate)
    ...
    if isempty(find(ismember(data.label,tmp_ch)==1, 1))==0 % channels already exists
        disp(['GIAC: channel ' tmp_ch{1,1} ' already exists and will not be created']);
    elseif isempty(find(ismember(data.label,tmp_ch)==1, 1))==1 % channels does not exists  
        data.label{end+1} = tmp_ch;        
...
% RS EDIT: REORDER LAYOUT TO MATCH CHANS IN DATA ? possibly not needed because it seems to just look up the appropriate channels regardless of order
