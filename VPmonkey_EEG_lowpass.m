...
% 
% 
%   apply lowpass filter to EEG data after ICA cleaning
%
...
%%%%%%% SELECT FILE TYPE %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
if s.lowpass == 95
    files = files(startsWith(files,'yesICA') & endsWith(files,'.set') & contains(files,'merged_EEG') );
elseif s.lowpass == 30
    files = files(startsWith(files,'ged_horiz LP_95') & endsWith(files,'.set') & contains(files,'merged_EEG') );
else
    error 'HAVENT SPECIFIED THE FILES FOR THIS FILTER BAND'
end
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
for f = 1 :nfiles
    ...
    %% LOWPASS FILTER
    EEG = pop_eegfiltnew( EEG, 'hicutoff', s.lowpass);

    %% SAVE FILTERED DATA
    saveName = ['LP_' num2str(s.lowpass) ' ' files{f}];
    EEG = pop_saveset(EEG, 'filepath', homedir  ...
        ,'filename', saveName ); 
