%
%   Use GED to try to remove weird horizontal artifact
% 
...
files = files(startsWith(files,'LP_95 yesICA') & endsWith(files,'.set') & contains(files,'merged_EEG') );
...
for f = 1 :nfiles
    ...
    %% COMPUTE GED 
    cfg = [];
    cfg.pre = EEG.times([1,end]);
    cfg.pos = [35, 110];
    cfg.phaselocked = true;
    cfg.reduce2rank = true;

    EEG = pop_rs_GED_ERCA(EEG,cfg);
    
    %% REJECT GED components ----------------------------------------------------------------------------------------------------------
    close all;
    %
    cfg = [];
    cfg.mode = 'gui';
    %
    cfg.gui.letsplot.channel = 'CZ';
    cfg.gui.letsplot.xlim    = [-0.2, 0.6];
%     ctemp.gui.letsplot.ylim    = [-20, 20];
    cfg.gui.freqs         = s.freqs;
    cfg.gui.comps2reject  = 1; 
    cfg.gui.arrangefigs   = false;
    %
    EEG = pop_rs_GED_reject(EEG,cfg);
    % ----------------------------------------------------------------------------------------------------------------------

    %% save .set
    saveName = [ 'ged_horiz ' EEG.filename(1:end-4) ];
